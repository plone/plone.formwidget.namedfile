...
class INamedFileWidget(IFileWidget):
    ...
    filename = schema.TextLine(
        title=u"Name of the underlying file", required=False)
    filename_encoded = schema.TextLine(
        title=u"Filename, URL-encoded", required=False)
...
class INamedImageWidget(INamedFileWidget):
    ...
    thumb_width = schema.Int(
        title=u"Thumbnail image width", min=0, required=False)
    thumb_height = schema.Int(
        title=u"Thuimbnail image height", min=0, required=False)
